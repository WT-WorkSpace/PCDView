...
<ul>
  ...
  <li><b>自定义标注属性</b>：配置目标框属性字段、历史帧显示模式，以及是否为空帧自动生成空 JSON 标注文件。</li>
</ul>
...
<h3>点云分割标注</h3>
<ul>
  <li><b>点云分割标注</b>：进入 3D 分割标注模式；左键旋转、平移和滚轮缩放等原有视图操作仍可使用。</li>
  <li><b>分割标签设置</b>：在 Tools 中配置标签 name、整数 key 和颜色；默认背景标签为白色，key 为 0。</li>
  <li>标注模式下，<b>右键点击</b>依次添加多边形顶点；右键点击首点附近闭合多边形。</li>
  <li>闭合后会弹出标签选择窗口，确认后多边形内点云变为对应标签颜色；后续框选可覆盖之前的标签。</li>
  <li>左下角图例显示标签颜色、key 和当前帧各标签点数。</li>
  <li>每次完成标注会自动保存同名 <b>.txt</b> 文件；格式为每行一个数字，对应点的标签 key。</li>
  <li><b>加载分割TXT</b>：在 Tools 中加载已有 txt 到当前帧，点数必须与当前点云一致，加载后可继续修改。</li>
  <li><b>退出分割模式</b>：在 Tools 中退出点云分割标注模式；没有未闭合多边形时按 <b>Esc</b> 也会退出。</li>
